...
- 中位数准确率约为54.88%，略高于随机猜测（50%）
- 有少数模型达到了较高的准确率（>60%），最高达到92.86%
- 红色虚线表示平均值，绿色虚线表示中位数
- 统计数据显示模型性能相对稳定，标准差为3.04%
...
```bash
cd infer_res/analyze_all_acc
python analyze_accuracy.py --logs-dir ../../logs --output results/accuracy_histogram.png
```

该脚本会自动分析 `logs/` 目录下所有训练模型的日志，提取验证集准确率并生成分布直方图。统计信息会同时保存为JSON文件（`accuracy_histogram_stats.json`）。
