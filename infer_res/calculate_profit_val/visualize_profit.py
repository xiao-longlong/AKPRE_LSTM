...
def plot_price_changes(data, output_file=None):
    ...
    close_prices = [close_prices[i] for i in sorted_indices]
    price_changes = [price_changes[i] for i in sorted_indices]
...
def plot_asset_comparison(data, output_file=None):
    ...
    # 按日期排序
    sorted_indices = np.argsort(trade_dates)
    trade_dates = [trade_dates[i] for i in sorted_indices]
    trade_values = [trade_values[i] for i in sorted_indices]
    ...
    if len(daily_data) >= 2:
        ...
        if first_close:
            ...
            # 按日期排序
            if hold_dates:
                sorted_indices = np.argsort(hold_dates)
                hold_dates = [hold_dates[i] for i in sorted_indices]
                hold_values = [hold_values[i] for i in sorted_indices]
...
def plot_accuracy_convergence(data, output_file=None):
    ...
    # 先按日期排序predictions
    sorted_predictions = sorted(predictions, key=lambda x: pd.to_datetime(x['date']))
